...
import { api, type AgentUpdatePayload } from '@/lib/api';
...
export default function EditAgentPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function loadAgent() {
      try {
        ...
        setForm({
          ...
          price_per_use: agent.price_per_use != null ? String(agent.price_per_use) : '',
          monthly_price: agent.monthly_price != null ? String(agent.monthly_price) : '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!validate()) return;
    if (!accessToken) {
      setErrors({ submit: 'You must be logged in to edit an agent.' });
      return;
    }
    ...
    try {
      ...
      await api.agents.update(id, payload, accessToken);
      router.push(`/agents/${id}`);
    } catch (err) {
      setErrors({ submit: err instanceof Error ? err.message : 'Failed to update agent' });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12 lg:px-8">
      ...
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-400">Update your agent&apos;s information</p>
      </div>
